...
def main():
    ...
    with terminal("rodis", *target) as t:
        t.wait("실시간 조회")
        t.send(b"/")
        t.wait("filter>")
        t.send(b"ctlprobe")
        ...
        t.wait("ALIVE")
        pid = c.live("juno")
        lab.inside("kill", "-TERM", pid)
        c.eventually(lambda: not c.live("juno"))
        t.wait("연결 끊김")
        t.save("disconnected")
        c.restart("juno")
        t.send(b"r")
        t.wait("실시간 조회")
        assert "연결 끊김" not in t.text()
        t.save("reconnected")
        t.resize(60, 19)
        t.wait("q 종료")
        t.save("60x19")
    check("rodis live stop/start, disconnected snapshot, reconnect and 60x19")
